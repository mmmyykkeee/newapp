...
(window.Modernizr = (function (a, b, c) {
  ...
  return (
    ...
    (e.testStyles = t),
    (g.className =
      g.className.replace(/(^|\s)no-js(\s|jQuery)/, "jQuery1jQuery2") +
      (f ? " js " + q.join(" ") : "")),
    e
  );
})(this, this.document)),
...
jQuery("document").ready(function () {
  ...
  if (Modernizr.touch) {
    ...
    jQuery(".mtmegamenu .mt-root").hover(
      function (e) {
        if (mega_menu != 1)
          jQuery(this).doTimeout(
            "fieldmenuhover",
            150,
            showMegamenuMenu,
            e.target
...
function showMegamenuMenu(el) {
  ...
  var _containerLeftPadding = parseInt(
    jQuery(".mtmegamenu").closest(".container").css("padding-left")
  );
  var _mainOffset = jQuery(".mtmegamenu").offset();
  var _menuPopupOffset = jQuery(el).closest(".mt-root").offset();
  var mLeft =
    _menuPopupOffset.left -
    _mpadding +
    (_containerOffset.left - _mainOffset.left) +
    _containerLeftPadding;
